...
resource "azurerm_app_service" "app-service-admin-primary" {
  name                = "${var.application_name}-admin-as-${var.environment}-${var.location}"
  ...
  site_config {
    ...
    linux_fx_version  = "DOCKER|${var.docker_registry}/${var.docker_container_admin}" #define the images to use for your application
...
resource "azurerm_app_service" "app-service-admin-secondary" {
  name                = "${var.application_name}-admin-as-${var.environment}-${var.backup_location}"
  ...
  site_config {
    ...
    linux_fx_version  = "DOCKER|${var.docker_registry}/${var.docker_container_admin}" #define the images to usecfor you application
